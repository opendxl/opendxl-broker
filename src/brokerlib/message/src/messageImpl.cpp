...
static int msgpack_sbuffer_write_callback(void *data, const char *buf, unsigned int len)
{
...
/******************************************************************************/
{
    ...
    msgpack_packer* pk = msgpack_packer_new(buffer, msgpack_sbuffer_write_callback);
    ...
    msgpack_pack_raw(pk,temp);
    msgpack_pack_raw_body(pk,temp ? message->messageId : NULL, temp);

    /* client ID */
    temp = strlen(message->sourceClientId);
    msgpack_pack_raw(pk,temp);
    msgpack_pack_raw_body(pk,temp ? message->sourceClientId : NULL, temp);

    /* source broker GUID */
    temp = strlen(message->sourceBrokerGuid);
    msgpack_pack_raw(pk,temp);
    msgpack_pack_raw_body(pk,temp ? message->sourceBrokerGuid : NULL, temp);
    ...
    for( i = 0; i < message->brokerGuidCount; i++ )
    {
        ...
        msgpack_pack_raw(pk, temp);
        msgpack_pack_raw_body(pk, message->brokerGuids[i], temp);
    }
    ...
    else
    {
        ...
        for( i = 0; i < message->clientGuidCount; i++ )
        {
            ...
            msgpack_pack_raw(pk, temp);
            msgpack_pack_raw_body(pk, message->clientGuids[i], temp);
        }
    }

    /* payload */
    msgpack_pack_raw(pk,message->payloadSize);
    msgpack_pack_raw_body(pk,message->payloadSize ? message->payload : NULL, message->payloadSize);
    ...
    if ( message->messageType == DXLMP_REQUEST )
    {
        ...
        msgpack_pack_raw(pk,temp);
        msgpack_pack_raw_body(pk,temp ? message->dxl_message_specificData.requestData->replyToTopic : NULL, temp);

        temp = strlen(message->dxl_message_specificData.requestData->serviceInstanceId);
        msgpack_pack_raw(pk,temp);
        msgpack_pack_raw_body(pk,temp ? message->dxl_message_specificData.requestData->serviceInstanceId : NULL, temp);
    }
    else if ( message->messageType == DXLMP_RESPONSE )
    {
        ...
        msgpack_pack_raw(pk,temp);
        msgpack_pack_raw_body(pk,temp ? message->dxl_message_specificData.responseData->requestMessageId : NULL, temp);

        /* Service instance ID */
        temp = strlen(message->dxl_message_specificData.responseData->serviceInstanceId);
        msgpack_pack_raw(pk,temp);
        msgpack_pack_raw_body(pk,temp ? message->dxl_message_specificData.responseData->serviceInstanceId : NULL, temp);
    }
    else if ( message->messageType == DXLMP_RESPONSE_ERROR )
    {
        ...
        msgpack_pack_raw(pk,temp);
        msgpack_pack_raw_body(pk,temp ? message->dxl_message_specificData.responseErrorData->requestMessageId : NULL, temp);

        /* Service instance ID */
        temp = strlen(message->dxl_message_specificData.responseErrorData->serviceInstanceId);
        msgpack_pack_raw(pk,temp);
        msgpack_pack_raw_body(pk,temp ? message->dxl_message_specificData.responseErrorData->serviceInstanceId : NULL, temp);
        ...
        msgpack_pack_raw(pk,temp);
        msgpack_pack_raw_body(pk,temp ? message->dxl_message_specificData.responseErrorData->errorMessage : NULL, temp);
    }
    ...
    if ( message->version > 0 )
    {
        ...
        for( i = 0; i < message->otherFieldsCount; i++ )
        {
            ...
            msgpack_pack_raw(pk, temp);
            msgpack_pack_raw_body(pk, message->otherFields[i], temp);
        }
    }
    ...
    if ( message->version > 1 )
    {
        ...
        msgpack_pack_raw(pk,temp);
        msgpack_pack_raw_body(pk,temp ? message->sourceTenantGuid : NULL, temp);
        ...
        for( i = 0; i < message->tenantGuidCount; i++ )
        {
            ...
            msgpack_pack_raw(pk, temp);
            msgpack_pack_raw_body(pk, message->tenantGuids[i], temp);
        }
    }
    ...
    if ( message->version > 2 )
    {
        ...
        msgpack_pack_raw(pk,temp);
        msgpack_pack_raw_body(pk,temp ? message->sourceClientInstanceId : NULL, temp);
    }
...
/******************************************************************************/
{
    ...
    if ( result.data.type != MSGPACK_OBJECT_RAW )
    {
        msgpack_unpacked_destroy(&result);
        return DXLMP_BAD_DATA;
    }

    *bufferOut = (unsigned char*)calloc(result.data.via.raw.size + (addNullTerminator ? 1 : 0), 1);
    if ( !*bufferOut )
    {
        msgpack_unpacked_destroy(&result);
        return DXLMP_NO_MEMORY;
    ...
    memcpy(*bufferOut, result.data.via.raw.ptr, result.data.via.raw.size);
    *bufferSize = result.data.via.raw.size + (addNullTerminator ? 1 : 0);
    ...
    *stringArraySize = 0;
    ...
    if( mpArraySize > 0 )
    {
        *stringArray = (char**)calloc(mpArraySize, sizeof(char*));
        ...
        {
            (*stringArray)[i] = (char*)calloc(result.data.via.array.ptr[i].via.raw.size+1,sizeof(char));
            if( !((*stringArray)[i]) )
            {
            ...
            memcpy((*stringArray)[i],
                result.data.via.array.ptr[i].via.raw.ptr,
                result.data.via.array.ptr[i].via.raw.size);
        }
